...
export default function EditProductPage() {
    ...
    const loadData = async () => {
      try {
        ...
        const vars: VariationForm[] = (product.variations || []).map((v: any) => {
          const selectedAttrs: Record<number, number> = {};
          (v.attribute_values || []).forEach((av: any) => {
            selectedAttrs[av.attribute?.id || av.variation_attribute_id] = av.id;
          });
          return {
            sku: v.sku,
            price: v.price,
  ...
  const handleCheckboxChange = (fieldId: number, option: string, checked: boolean) => {
    let current: string[] = [];
    try { current = fieldValues[fieldId] ? JSON.parse(fieldValues[fieldId]) : []; } catch { current = []; }
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="text" value={origin} onChange={(e) => setOrigin(e.target.value)} className={inputClass} />
                  </div>
                </div>
                <div>
                    ...
                    <div key={field.id}>
                      ...
                      {field.type === 'checkbox' && (
                        <div className="flex flex-wrap gap-3 mt-1">
                          {field.options?.map((opt) => {
                            let current: string[] = [];
                            try { current = fieldValues[field.id] ? JSON.parse(fieldValues[field.id]) : []; } catch { current = []; }
                            return (
                  ...
                  <div key={index} className="border border-zinc-700 rounded-lg p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      <div>
                        ...
                        <input type="text" value={variation.sku} onChange={(e) => updateVariation(index, 'sku', e.target.value)} className={inputClass} />
                      </div>
                      <div>
                        <label className={labelClass}>Fiyat (₺)</label>
                        <input type="number" step="0.01" value={variation.price} onChange={(e) => updateVariation(index, 'price', e.target.value)} className={inputClass} />
                      </div>
                      <div>
                        <label className={labelClass}>Stok</label>
                        <input type="number" value={variation.stock} onChange={(e) => updateVariation(index, 'stock', parseInt(e.target.value) || 0)} className={inputClass} />
                      </div>
                    </div>
