...
<div class="flex items-center gap-4 mb-8">
    ...
    <div>
        <h3 class="text-xl font-semibold text-white">Yeni Ürün</h3>
        <p class="text-sm text-zinc-500 mt-1">Yeni bir kahve ürünü oluştur</p>
    </div>
</div>
...
<form action="/admin/products" method="POST">
    ...
    <div class="grid grid-cols-3 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4"
                                  class="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-2.5 text-white placeholder-zinc-500 focus:outline-none focus:border-[#E8D5B7] text-sm resize-none"
                            ...
                            <div class="flex flex-wrap gap-3">
                                ...
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="fields[{{ $field->id }}][]" value="{{ $option }}"
                                           class="w-4 h-4 rounded border-zinc-600 bg-zinc-800 text-[#E8D5B7]">
                            ...
                            <div class="flex flex-wrap gap-4">
                                ...
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="fields[{{ $field->id }}]" value="{{ $option }}"
                                           class="w-4 h-4 border-zinc-600 bg-zinc-800 text-[#E8D5B7]">
                                    <span class="text-sm text-zinc-300">{{ $option }}</span>
                                </label>
                                @endforeach
                            </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-zinc-900 rounded-xl border border-zinc-800 p-6">
                <h4 class="text-sm font-semibold text-white mb-3">Varyasyon Tipleri</h4>
                @forelse($attributes as $attribute)
                <div class="mb-3">
                    <p class="text-xs text-zinc-400 mb-2">{{ $attribute->name }}</p>
                    <div class="flex flex-wrap gap-2">
                        @foreach($attribute->values as $value)
                        <span class="px-2.5 py-1 bg-zinc-800 rounded-md text-xs text-zinc-300">
                            {{ $value->value }}
                        </span>
                        @endforeach
                    </div>
                ...
                @empty
                <p class="text-xs text-zinc-500">Henüz varyasyon tipi eklenmemiş.</p>
                @endforelse
            </div>
...
function addVariation() {
    ...
    div.innerHTML = `
        ...
        <button type="button" onclick="removeVariation(${variationIndex})"
